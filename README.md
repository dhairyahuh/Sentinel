...
**Financial crime doesn't hide in transactions. It hides in patterns.**

*Submitted to Youth Economy Lab (YEL) IGDTUW Chapter*  
**Track 2 — Fraud Detection & Financial Crime Prevention**  
**Problem Statement 5 — Tracing Financial Crime Across Patterns**

<br/>

**Sentinel maps financial crime rings, campaign networks, and kill-chain patterns across thousands of transactions — then trains an ensemble defence that catches cross-account patterns and adapts as criminal tactics evolve.**

<br/>

![Python](https://img.shields.io/badge/python-3.9%2B-blue)
![Tests](https://img.shields.io/badge/tests-283%20functions%20%C2%B7%2012%20modules-green)
![Crime patterns](https://img.shields.io/badge/crime%20patterns-67%20across%209%20families-red)
...
## YEL Hackathon Alignment

### Problem Statement: Tracing Financial Crime Across Patterns
...
1. **Multi-Account Mule Ring Detection:** Traces transactions funnelling across disparate accounts into coordinated beneficiary clusters and cash-out networks.
2. **Coordinated Campaign Grouping:** Groups transactions generated in the same criminal attack waves across accounts and rails.
3. **Causal Trailing Windows & Account Graphs:** Feature engineering uses strict past-window historical aggregates across the payment network, eliminating single-transaction myopia without future leakage.
4. **Pattern Taxonomy (67 Patterns, 9 Families):** Maps the entire attack lifecycle (reconnaissance, pretext, authorisation, settlement, layering, cash-out).
5. **Adaptive Arms Race Co-Evolution:** As criminals mutate transaction parameters to evade detection, Sentinel discovers surviving mutations and feeds them back into the crime pattern map.
...
```mermaid
flowchart LR
    I["<b>1 · MAP</b><br/>67 crime patterns<br/>9 criminal families<br/>machine-readable taxonomy"]
    G["<b>2 · SIMULATE</b><br/>synthetic payments<br/>agent context bundles<br/>call transcripts"]
    D["<b>3 · DETECT</b><br/>causal features<br/>5 guards<br/>stacked ensemble"]
    L["<b>4 · EVOLVE</b><br/>red mutates for profit<br/>blue answers by cost"]

    I -->|"pattern mix,<br/>observable signals"| G
    G -->|"labelled traffic,<br/>3 data planes"| D
    D -->|"frozen detector<br/>+ threshold"| L
    L -.->|"<b>surviving mutations become<br/>new pattern entries</b>"| I
...
| [Pillar 1 — Map](#pillar-1--identify) | The pattern taxonomy as executable contract |
| [Pillar 2 — Simulate](#pillar-2--generate) | Three data planes, and measured fidelity |
| [Pillar 3 — Detect](#pillar-3--defend) | Three layers, and how they are evaluated |
| [Adaptive Evolution](#the-closed-loop) | Economically motivated co-evolution |
...
The core artifacts for the YEL submission:

| # | Artifact | Notes |
|:--:|---|---|
| **1** | **Code repository** | Complete pipeline: pattern taxonomy, multi-plane simulation, ensemble detection, adaptive loop, and console. Organised, documented, reproducible from a config and seed. |
| **2** | **Solution walkthrough** | [`docs/Solution-Walkthrough.docx`](docs/Solution-Walkthrough.docx) — Seven sections covering patterns mapped, simulation engine, detection models with empirical intervals, and production feasibility. |
| **3** | **Working prototype** | Seven-route operator console with demo mode, transaction stream, and transaction inspector with mule ring / campaign graph drill-downs. Run locally with `docker compose up` → <http://127.0.0.1:5173>. |
...
The Sentinel console runs as a client-side static application or with the live API server. Every chart, table, drill-down and the
guided tour work, and keep working with the network disconnected once loaded in Demo Mode.

The three controls that *compute* — `RUN A ROUND NOW`, the `/deploy` latency probe, and
scoring a transaction — connect to the FastAPI backend. Run the stack below to make them live.
