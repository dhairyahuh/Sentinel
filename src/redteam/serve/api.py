...
def create_app(bundle_path: Path = DEFAULT_BUNDLE,
    ...
    app = FastAPI(
        title="Sentinel — Financial Crime Pattern Intelligence",
        version="1.0.0",
        description=(
            "Financial crime pattern detection ensemble trained and evaluated on cross-account "
            "synthetic payment data for YEL Track 2 Problem 5. Scores payments against crime patterns, "
            "reports precision and recall on a held-out test set, and exposes audit artefacts. "
            "See `GET /model-card` for the full statement of intended use and limitations."
        ),
    )
