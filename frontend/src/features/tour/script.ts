...
export const SCRIPT: Step[] = [
  {
    ...
    target: 'tour-stream',
    caption:
      'Transactions arriving and being scored against cross-account crime patterns. Every row here was scored by the trained ensemble ' +
  ...
  },
  {
    ...
    caption:
      'One alerted transaction, opened. Reason codes in English, cross-account mule ring and campaign siblings, every model input against what ' +
  ...
  },
  {
    ...
    caption:
      'The crime pattern map against the stages of an attack. The shading is measured detection recall ' +
      'joined from the detector — not intent, not coverage claimed. An unshaded square is a ' +
      'crime pattern this run never simulated, which is a different and more honest thing to admit ' +
  ...
  },
  {
    ...
    caption:
      'The adaptive arms race. Three lines per round: recall before crime patterns mutated, after the best evasion mutation ' +
      'landed, and after the detector retrained and adapted in response. The gap between the first two is what the ' +
      'evasion cost; the gap between the second and third is what the adaptive defence won back. Those ' +
  ...
  },
  {
    ...
    caption:
      ...
      'between the two numbers is the levers, not sampling. This is the mutation evaluation; ' +
  ...
  },
  {
    ...
    caption:
      'And this is the arrow closing. A criminal mutation that evades, survives validation against the ' +
      'pattern library schema, and is written back as a first-class pattern — which the next run ' +
      'simulates and trains on from the start. The system that was attacked is now looking for ' +
      'the mutation. That is the closed loop, and it is the whole submission.',
    hold: 14000,
  },
  {
    route: '/deploy',
