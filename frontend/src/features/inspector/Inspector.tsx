...
function Siblings({ siblings }: { siblings: TransactionDetailSiblings }) {
  ...
  if (!siblings.available || (campaign.length === 0 && ring.length === 0)) {
    return (
      <Note>
        No other transaction in the test window shares this one&apos;s campaign or mule ring. For a
        legitimate transaction that is expected; for an attack it means the pattern generator produced it
    ...
    <div className="space-y-3">
      {campaign.length ? (
        <SiblingList
          ...
          note="Cross-account transactions generated in the same criminal attack wave."
          rows={campaign}
        />
      ) : null}
      {ring.length ? (
        <SiblingList
          title="Money laundering mule ring"
          note="Multi-account transactions funnelling into the same beneficiary cluster."
          rows={ring}
        />
      ) : null}
    </div>
