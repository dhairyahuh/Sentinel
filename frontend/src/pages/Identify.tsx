...
export default function Identify() {
  ...
  return (
    <div className="grid h-full min-h-0 grid-cols-[1fr_340px] grid-rows-[1fr_auto] gap-2 p-2">
      <Panel
        id="tour-killchain"
        title="Crime pattern map"
        source="src/redteam/identify/attack_library.yaml"
        subtitle={
          data ? (
            <>
              {count(data.counts.total)} crime patterns · {count(data.counts.simulated)} simulated ·{' '}
              {count(data.counts.measured)} with measured detection recall
            </>
          ) : null
        }
        caveat="Shading is measured recall, joined from defend_per_vector_recall.csv. An unshaded cell is a pattern this run never simulated — not a pattern the detector failed on. Those are different, and the residual-risk table below ranks them accordingly."
        scroll
      >
        {isLoading ? (
          <PanelLoading />
      ...
      <Panel
        title={vector ? vector.name : 'Pattern detail'}
        source="src/redteam/identify/attack_library.yaml"
      ...
      <Panel
        ...
        subtitle="Ranked by a-priori risk that this run has not shown the detector covers."
        caveat="Residual is the pattern library's risk score discounted by measured recall. A pattern never simulated keeps its full risk, because nothing has been shown about it — which is why the top of this table is mostly things not yet simulated rather than detection failures."
  ...
  return (
    <div className="space-y-2 p-3">
      <p className="text-2xs leading-relaxed text-fg-muted">
        Each square is one crime pattern vector, placed at every attack lifecycle stage it touches.
      </p>
