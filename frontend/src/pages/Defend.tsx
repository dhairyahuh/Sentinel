...
export default function Defend() {
  ...
  return (
    <div className="grid h-full min-h-0 grid-cols-2 grid-rows-2 gap-2 overflow-auto p-2">
      ...
      <Panel
        id="tour-pervector"
        title="Detection rate by crime pattern"
        source="defend_per_vector_recall.csv"
        subtitle="Every simulated crime pattern, with a Wilson interval on its detection recall."
        caveat="Rows with n_sufficient = 0 have too few positives in the test window for their recall to mean much. They are shown rather than hidden, because which crime patterns we cannot measure is itself a finding."
        scroll
      >
      ...
      <Panel
        title="Against baseline detectors"
        source="defend_baselines.csv"
        subtitle="The pattern ensemble beside a single rule, an expert rule set and logistic regression."
        caveat={
          <>
      ...
      <Panel
        title="False positive burden and financial impact"
